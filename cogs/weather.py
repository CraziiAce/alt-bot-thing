    ...
    async def weatherzip(self, ctx, zip_code: str):
        ...
        async with aiohttp.ClientSession() as session:
            url = (
                ...
                + zip_code
                + "&appid=168ced82a72953d81d018f75eec64aa0&units=imperial"
            )
            async with session.get(url) as response:
                weather_response = await response.json()
                if weather_response["cod"] != 200:
                    await ctx.send(
                        f"An error ocurred: `{weather_response['message']}`. If the issue persists, please contact a support team member with this error code: `{weather_response['cod']}`"
                    )
                else:
                    currentUnix = time.time()
                    localSunrise = (
                        weather_response["sys"]["sunrise"]
                        + weather_response["timezone"]
                    )
                    sunriseTime = datetime.datetime.utcfromtimestamp(localSunrise)
                    localSunset = (
                        weather_response["sys"]["sunset"] + weather_response["timezone"]
                    )
